...
public class UserHoverTag : MonoBehaviour {
	...
	void Start () {
        GameObject.Find("userNameText").GetComponent<Text>().text = GameObject.Find("UserStats").GetComponent<UserStats>().username;
	...
	void Update () {
        for (int i = 0; i < GameObject.Find("Stratos").GetComponent<Player_Info_Ingame>().playersLeaderboard.Length; i++)
        {
            if (!GameObject.Find("Stratos").GetComponent<Player_Info_Ingame>().playersLeaderboard[i].GetComponent<Player_Info_Ingame>().isLocalPlayer)
                GameObject.Find("Stratos").GetComponent<Player_Info_Ingame>().playersLeaderboard[i].gameObject.transform.Find("userName/userNameTexture/userNamePosition").GetComponent<Text>().text = GameObject.Find("Stratos").GetComponent<Player_Info_Ingame>().leaderboardPosition.ToString();
        }
    }
}
